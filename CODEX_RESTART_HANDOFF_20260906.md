## 最新：Stormの粒子小型化、表示と検出を画面全体へ（2026-09-07）

845f641を候補へpush後、07:38に新表示。trial `kids_trial_20260907_073818_662974100`、run `particle_storm_mvp_20260907_073817_276098`。Manager20872/Storm22608/wrapper7520/外側22980、FIRST_FRAME3.688秒、露出-4/zoom176一時適用。26秒時点camera failure/reopen0、直近29.9fps、描画/検出例外なし。Xiaomi client全域一致、下端3点Storm。最終版230テストは描画Python失敗0/skip1、3.11失敗0/skip5。右側の最新操作パネルを使用。次は中央→左右端の指/カーソル一致と生映像の端を約1分だけ人間に確認し待つ。07:53頃自動終了予定なので再開時はログを再読取。目視合格は未回答。

最新指示「やはり、表示も検出もディスプレイに合わせるだけでいい」を採用し、途中の「ずれを残す」は上書き。PointsMaterialのsize15→7.5と、読み取り幅640/表示比に合う高さ360を適用。1280×720入力を16:9のまま1920×1080全域へ配置し、検出とカーソルも同一layoutで合わせる。直前のZ維持、共通カメラ/zoom176/反転は維持。粒径・画面端の位置一致・グーパー・奥行きの人間確認は未回答。旧trial072307は07:34:43に通常終了、exit0/共有メモリ不在。詳細はPARTICLE_STORM_DEPTH_FIX_20260907.md。

## 最新：Particle Stormの奥行き維持を最小修正（2026-09-07）

修正コミット6477ccbを候補へpush後、07:23に新Stormを起動。trial `kids_trial_20260907_072307_553372400`、run `particle_storm_mvp_20260907_072306_138819`。Manager25176/Storm12432/wrapper14268/外側17104、FIRST_FRAME4.078秒、従来の露出-4/zoom176を一時適用。26秒時点でcamera failure/reopen0、直近描画27.96fps、ログに描画/検出例外なし。Xiaomi client全域と下端3点はStorm。新しい操作パネルを右側に開いた。15分上限で07:38頃終了予定なので、再開時は先にログを再読取する。

次に人間へ提示する確認は1まとまり：中央で拳（赤カーソル）を3分保ち、少し左右へ動かして開掌に戻す。厚みと従来の追従/反発が維持されればOK、板状化/追従崩れ/反発不良ならNG。まだ人間未回答であり合格にしない。USBを抜かせず、子供/実2人の保留も混ぜない。

ユーザーの最新依頼でStormのZ収束修正を優先。`particle_storm_2.py` の手の力を合算した後に `forces[:, 2] = 0.0` を加え、手の力はXYだけに限定した。3D距離によるXYの強さ・近接透過率、既存Zノイズ/速度/減衰/境界は維持。寿命・再生成や他シーン・共通カメラの変更なし。

実コールバックの180秒相当数値試験でZ標準偏差は旧584.735→0.039、新584.735→585.201。新3試験は旧コード2失敗から全通過へ。全回帰229件：描画Python失敗0/skip1、Python3.11失敗0/skip4。詳細は `PARTICLE_STORM_DEPTH_FIX_20260907.md`。修正後の実画面・操作感は人間確認待ちであり、過去30分合格で代用しない。直前の戻し先は8518965。USB自動復帰の実装はまだ行っていない。子供/実2人は別の保留を維持。

## 最新：カメラだけの再生成後の目視復帰合格（2026-09-07）

ユーザーがrun065838の再生成後、Xiaomiの生映像で手が動きC922nも点灯する確認にokと回答。同一プロセスPID18364内での明示的な一度の再生成（4.203秒）の数値復帰と目視復帰が揃った。製品自動復帰の実装・抜差し合格には読み替えない。原因調査の区切りとして診断へ通常stopを送り、elapsed246.578秒でexit0/cleanup_okを確認。診断表示は終了済み。

確定した経路はUSB後のread=True/非空黒バッファに対してgrab=True/retrieve=Falseとなり、現relayが連続失敗数を0へ戻してreopenへ到達しないこと。次に実装する場合の最小範囲は物理DShow取得の両API成否確認を定常取得・初回制御値・起動FPS診断へ反映し、正常な暗所を拒否せず、取得失敗継続時間と既存再生成待ちの合計を評価すること。製品コードは本調査で変更していない。

自動復帰は未実装・未合格。子供確認と実2人同時操作はそれぞれ参加者待ちのHuman Check Required。今の診断の再起動・抜差しを繰り返し要求しない。

## 最新：目視待ちのままカメラ再生成を一度実施（2026-09-07）

新run065838でユーザーがUSBを戻したと回答。実PID18364のまま全黒約1fpsを再確認。elapsed157.984/159.984秒のread=True、158.984/161.000秒のgrab=True/retrieve=False、各約1秒、いずれも非空全黒バッファ。先行runと同じAPI成否差を再現。

162.000秒に明示reopenを一度要求し、166.203秒に成功（4.203秒）、新capture object、attempt1、release_errorなし。露出-4/zoom176の実値保持。168.281/170.313秒sampleで非黒フレーム32〜33枚/約2秒、同じPID継続。製品コードや設定の追加変更なし。次はUSBを触らず、Xiaomiの生映像で手が動きC922が点灯しているかを人間に確認して待つ。まだ再生成後の目視を合格にしない。現在の30分診断上限は07:28:38頃。自動復帰未実装・未合格、子供/実2人は別の参加者待ち保留。

## 最新：診断再準備後の生映像・LEDを人間確認（2026-09-07）

ユーザーが新run065838の生映像の動きとC922点灯の確認にokと回答。今回の正常な開始状態を目視合格とし、旧runの再生成後の目視やUSB自動復帰の合格にはしない。最新sample elapsed81.844秒、実PID18364維持、非黒フレーム33枚/2.047秒、reopened=false、終了なし。

次の操作は同じポートでUSBを5秒間抜いて戻し「戻した」と回答する一まとまり。回答後にagentが同runの状態を再読取りし、必要ならprobeで取得APIを比較、その後reopenを一度だけ送る。カメラ再生成後のXiaomi生映像の動き・C922点灯を人間に確認して待つ。手動診断による復帰を自動修正の合格にしない。製品変更なし。子供と実2人確認は保留維持。
...
## 参加者が必要な実機確認の保留（2026-09-07）

ユーザーは子供の参加、続いて2人目の参加について、どちらも現在は不可能と回答。子供による遊びの成立と2人同時操作は Human Check Required / 参加者待ちとして保留する。成人1人の両手確認や約1.5mの合格で代用しない。現在の参加可否を繰り返し尋ねず、参加可能になった際に再開する。独立して進められる残項目はUSB自動復帰NGの切り分け、終了時例外の精査など。12時間試験や本番合格への昇格は行わない。

## 2026-09-07 06:23 更新：Spheres距離確認

ユーザーがC922nから約1.5m、同じ照明・露出・ズームでの指先の白い円の位置一致・追従と粒子反応の確認に ok と回答。今回の成人・約1.5m条件を目視合格とする。子供・2人同時・現場照明・他の距離へ合格を拡張しない。次は子供が今参加可能かを確認して待つ。USB自動復帰NGは継続。
