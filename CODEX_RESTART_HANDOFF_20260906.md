## 最新：9caa62aを合格基準に固定、次の1項目は30分連続の対応案（2026-09-07）

ユーザーの指示で `9caa62ae1832c5bb600476ebff8567d40354083f` を固定。タグ `codex/storm-visual-baseline-20260907` を作成・pushし、ローカル/リモートでSHA一致を確認。Stormの粒径・配置・XY操作・奥行きを以後変更しない。基準版の成人基本目視合格であり、MVP全体/子供/実2人/USB/長時間の合格ではない。候補HEADの資料更新とタグの基準点を混同しない。

未完了を照合し、コード変更0の現基準版30分連続試験を次の1件に選んだ。対応案は `MVP_NEXT_STEP_PLAN_20260907.md`。既存起動引数だけ31分上限にし、露出-4/zoom176の一時適用完了から正味30分を確保する。新しい実機試験はまだ開始していない。USB自動復帰と終了時例外は重要だが共通基盤へ影響するので今回の低リスク1件には選ばない。子供・実2人の参加待ちは別管理。

旧trial075145は08:06:56にduration_reached/exit0/900.859秒/promotion1で終了済み。今回の読取で対象Pythonと共有メモリ不在。現在Storm表示なし。資料とタグ以外の変更なし。次に実行指示があれば計画の実行順から進め、完了済み目視を重複要求しない。

## 最新：Stormの上下端も目視合格、今回修正の基本目視確認完了（2026-09-07 08:03）

ユーザーが開掌を上端→下端へ動かして指先とカーソルが重なる確認にokと回答。中央・左右端・上下端の位置一致、グーパーの吸引/反発、継続吸引後の厚み維持は成人目視確認済み。今回の修正の基本目視確認を完了とする。粒径の好みの明示回答、実際の子供、実2人、変更後30分/12時間、USB自動復帰まで合格にしない。

同trial075145を維持。648.969秒（08:02:34）で同一Storm/promotion1、camera failure/reopen0。製品/設定変更なし、記録のみ。表示は08:07頃の15分上限まで継続。次に継続するときは終了ログとPID/共有メモリの解放を確認し、変更後の30分試験を別途扱う。15分の2回合算を30分連続へ代用しない。完了した目視項目は繰り返し要求しない。

## 最新：Stormの継続吸引後の奥行き維持を目視合格（2026-09-07 08:00）

ユーザーが「そのまま拳の吸引をあと約2分続け、厚みが残ればOK」にokと回答。前回から約6分経過し、同trial075145は518.187秒で同一Storm/promotion1。中断の申告なし。人間の結果報告に基づき、3分相当の継続吸引後も厚みが残る確認を合格とする。ログは経過時間と同一プロセスを補強するだけで、拳の連続保持を自動測定したとはしない。

camera failure/reopen0、直近25.06fps、読取ログに描画/検出例外なし。製品コード/設定変更なし。次は表示比変更後に未確認の上下端の指/カーソル一致だけを30秒確認して待つ。中央/左右/グーパー/厚みを繰り返し要求しない。08:07頃終了予定。変更後30分/12時間、子供、実2人、USB自動復帰は別の未完了として維持。

## 最新：Stormの厚みの見え方OK、3分継続は未確認（2026-09-07 07:54）

ユーザーが奥行き確認にokと回答。厚みの見え方の成人目視OKとして記録する。ただしログは同trial075145のelapsed136.312秒（07:54:01）であり、再起動からまだ3分未満。冒頭の「約3分の吸引後も合格」という応答は再読取後に訂正した。3分連続吸引完了・30分・12時間の合格へ拡張しない。

同一Storm3352/promotion1、camera failure/reopen0、直近24.4fps、読取ログに描画/検出例外なし。表示・設定・コードは維持。次は同じ画面で拳の吸引をさらに約2分続け、厚みが残るかだけ確認して待つ。ユーザーが途中で手を外していた場合、3分連続は未確認として保持する。子供/実2人/USB自動復帰は別に保留。

## 最新：Stormのグー/パー合格、奥行き確認用に再表示（2026-09-07 07:52）

ユーザーが中央でグー/パー3回、赤で吸引/水色で反発する30秒確認にokと回答。表示比変更後の成人ジェスチャー確認を合格とする。粒径の好みや3分吸引後の奥行きの合格へは拡張しない。

旧trial073818は残り時間が3分に満たないため07:51:42に通常終了。exit0/promotion1、対象Python/共有メモリ不在を確認。777.703秒時点のcamera failure/reopen0。同じ製品コード/条件で新trial `kids_trial_20260907_075145_394133100`、run `particle_storm_mvp_20260907_075144_013367` を起動。Manager21520/Storm3352/wrapper13784/外側10360、FIRST_FRAME3.782秒、露出-4/zoom176実値一致（保存なし）。15分上限は08:07頃。次は中央で拳・赤カーソルの吸引を約3分保ち、最後に少し左右へ動かして厚みが維持されるかだけ人間に確認して待つ。製品変更・新規テストなし。子供/実2人/USB自動復帰/長時間は別に保留。

## 最新：Stormの中央・左右端の表示と検出一致を目視合格（2026-09-07 07:50）

ユーザーが直前の「中央→左端→右端、生映像が両端まであり指先とカーソルが重なる」確認にokと回答。この範囲の成人目視を合格として記録。上下端/全四隅、粒径の好み、3分吸引後の奥行き、グーパー切替の個別合格へは拡張しない。

同trial073818を継続。687.171秒でpromotion1、camera failure/reopen0、直近26.26fps、読取ログに描画/検出例外なし。次は中央でグー/パーをゆっくり3回、赤カーソルで吸引/水色で反発するか30秒だけ人間確認して待つ。07:53頃の自動終了まで残りが短いため、3分の奥行き確認をこの時点で重ねて要求しない。次回は再読取して終了済みなら同条件で再起動する。子供/実2人/USB自動復帰/長時間は保留。
