...
通常起動ではAcer内の操作UIも有効になる。起動ログの `[Operator]` 行のURLを `#token=` 以降も含めてブラウザで開く。露出・ズームを映像へ適用し、確認できた値をJSON保存できる。MacBookとの接続方式はAcerホットスポット／Bluetooth PAN／同じWi-Fiからまだ未選択。選んだ接続のAcer側IPv4を指定する手順は [OPERATOR_PANEL.md](OPERATOR_PANEL.md) を参照。MacBookからの実通信は未確認。
...
最新の`91b3200`では自然復帰と正常な切替の回数も分離し、全回帰120件がPython 3.11と既存映像venv 3.12.10の両方で成功した。前の版のC922n実30分は正常終了・資源残留なしだったが、途中の切替とEsc復帰を含み、30分無中断は未合格。最新の試験結果と限界は [継続検証記録](WINDOWS_VENV_RUNTIME_CHECK_20260906.md) を参照。旧94件とpreflightの根拠は [P1完了監査](P1_CANDIDATE_COMPLETION_AUDIT_20260906.md) に保持する。

Macの最初の疎通は、映像を止めず `Check Mac Connection.cmd --bind <Acerの接続先IPv4>` でカメラなしの確認ページを開ける。10分で自動終了する。Acer内のHTTP成功はMacからの通信成功と分ける。詳しい操作は [OPERATOR_PANEL.md](OPERATOR_PANEL.md) を参照。
...
- 確認済み: 既存映像用Pythonのpreflight、実C922nの取得と初回描画、操作を含む30分運転、修正後の同一dots20回交代とEsc復帰3回、終了後の所有PID/共有メモリ解放。無中断30分・異種シーン・実USB復帰は別確認。
...
- `HUMAN_CHECK_REQUIRED`: 実映像と指先の5点一致、複数人・子供の遊びやすさ、無中断30分、異種シーンの切替、USB再接続、選んだ接続方式でのMacBook操作。短時間の成功だけでは合格としない。
