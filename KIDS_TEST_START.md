...
最新の本番一覧省略・カメラ所有の追加修正後は全回帰94件が成功し、上記の映像Pythonで実際の `Start Kids Test.cmd --check` も成功した。実カメラは開いておらず、これから人間が実映像・長時間動作を確認する段階。[P1完了監査](P1_CANDIDATE_COMPLETION_AUDIT_20260906.md)に根拠と残る確認をまとめた。
...
- `HUMAN_CHECK_REQUIRED`: 実映像と指先の5点一致、複数人・子供の遊びやすさ、30分、切替反復、USB再接続、選んだ接続方式でのMacBook操作。短時間の成功だけでは合格としない。
