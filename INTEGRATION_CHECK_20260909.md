...
### 最新確定: 45分継続終了、残り2操作を別途補完

再試験は2026-09-09 03:39:31開始、2700.000秒継続し、監視issues=[]。Next34・Back34（全17シーンを両方向2周）、Safe/解除各2、Restart/直接選択各1の74操作を時間内完了。終了前50秒は新操作を送らない試験側制限と実際のポーリング間隔により最後のRestart/直接選択が残り、機械判定は `incomplete_at_time_limit`。これを76操作完走と改変しない。

完走まで進める指示に基づき、時間枠外の補完としてPrism Restart（6.078秒、launcher28692→33264）とFractal直接選択（10.079秒、33264→18112）を既存APIで実施して完了。これで計画76操作の確認を終えた。前の15分と今回45分は別試験であり連続60分とは記録しない。

- カメラ取得失敗・再接続0、フレーム最大間隔0.204秒、監視stderr空。270資源サンプル。全17シーンのprocessed_render_fpsを記録。Skeleton7.56〜8.11、Spheres8.04〜8.12と低いシーンあり、滑らかさの合格は自動判定しない。
- Manager private666701824→666697728bytes、handles765→765、GDI23/USER18一定。永続overlay PID14808 private753115136→759332864bytes、handles662→661、GDI20/USER13一定。CPU時間差はManager343.859秒、overlay1661.469秒（2700秒枠、CPU秒は累積で増加自体は異常でない）。
- 初期化後の新規シーン最大private増加231.01MiBはSkeleton PID8740（710012928→952242176bytes）。監視の持続増加閾値未満だが、増加を隠さず残す。今回の短い滞在だけでリークの有無を断定しない。
- 補完後、試験中FIRST_FRAMEが記録された旧scene/launcher PIDを現プロセスと照合し旧Pythonは残っていない。2件の同番号はpwsh/svchostへ再利用されており旧シーン残留ではない。現在のFractalと常駐Manager/overlayは意図して維持。
- 2画面列挙はDISPLAY1主1920×1080(0,0)、DISPLAY5拡張1920×1080(1920,0)で本番設定一致。試験後の実際の画面見え方・無人中の状態漏れ・GPUメモリは未確認。子供と実際の複数人操作は対象外。
- 根拠: `test_reports/final_endurance_45m_retry_20260909/{summary.json,analysis.json,resource_review.json,supplemental_operations.json,runtime.jsonl,events.jsonl}`。製品基準2a6a116に対する差分はこの記録だけ。今回の試験操作は終了し自動監視も停止する。

### 最新: 15分完走、追加45分は予定外Backで中断

ユーザーに「03:30頃、Backを操作しましたか？」と確認し、「はい」と回答を受領。予定外Backの原因はユーザーの手動操作と確認した。製品の誤動作とは扱わず、手動介入による試験条件変更として中断を分類する。ただし45分完走や残り32操作の合格へは繰り上げない。
