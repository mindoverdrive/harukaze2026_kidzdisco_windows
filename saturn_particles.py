...
class SaturnParticlesApp:
    ...
    def detect_hands(self):
        ...
        try:
            ...
            self.cam_tex.data[:] = cv2.resize(frame_rgba, (640, 480))
            self.cam_tex.update_range((0, 0, 0), self.cam_tex.size)
            
            # MediaPipe - ensure strictly increasing timestamp
            mp_img = mp.Image(image_format=mp.ImageFormat.SRGB, data=cv2.flip(frame_rgb, 1))
            ...
            if result.hand_landmarks:
                ...
                for i, hand_lms in enumerate(result.hand_landmarks):
                    ...
                    # World Coords
                    ix = (index_tip.x - 0.5) * 1600
                    iy = -(index_tip.y - 0.5) * 1200
                    iz = -(index_tip.z) * 1200
                    ...
                    hand_info = {
                        'index_tip': np.array([ix, iy, iz], dtype=np.float32),
                        'pinch': is_pinch,
                        'openness': openness,
                    }
                    ...
                    if i < len(self.cursors):
                        ...
                        self.cursors[i].local.position = (ix, iy, iz)
