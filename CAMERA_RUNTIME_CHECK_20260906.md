...
**最新の継続検証:** `270b315`で30分運転が正常終了・対象PID/共有メモリ残留0。次へ2回とEsc復帰2回を含み、単一シーン無中断は未合格。続いて自然復帰を切替成功に数える問題を`91b3200`で修正し、全回帰120件がPython 3.11と映像venv 3.12.10の両方で成功した。修正後の実20回切替も完走し、Esc復帰3回を成功回数から除外した。最新の結果は [Windows映像venvの継続検証記録](WINDOWS_VENV_RUNTIME_CHECK_20260906.md) の実試験IDと照合する。以下の112件・94件・84件と旧短時間試験は、それぞれ各版の履歴。

**最新の優先順位:** 部屋の暗さを含むFPSの追加調査はユーザー指示で後回し。露出・ズームは現場でMacのUIから調整する導線を優先し、今の作業とMac接続準備を並行する。以下は既に完了した観測であり、暗所の影響だけで全事象を説明した記録ではない。

**前段階のコード確認:** 本番一覧省略とカメラ初期設定・解放の二重失敗を追加修正し、全回帰94件が9.256秒で成功、終了コード0。既存映像Python 3.12.10による実際の `Start Kids Test.cmd --check` も成功した（`test_reports/kids_preflight_20260906_101821.json`、不足依存なし）。この確認ではカメラ・ウィンドウを開いていない。[P1完了監査](P1_CANDIDATE_COMPLETION_AUDIT_20260906.md)を参照。

**09:30の実機確認（前段階）:** 初回フレーム取得後に露出-5が-4へ戻る現象を再現し、取得後の1回補正・再読戻しを実装。実Managerで30.719秒、約30fps、途中終了なし、正常終了を確認。当時の回帰は84件。[露出戻りの再現と修正記録](C922_EXPOSURE_DRIFT_20260906.md)は今回の追加修正前の実測。下記16/30fpsの「原因未確定」は切り分け途中の履歴。
...
- MacBookからの実通信は未確認。接続方式はAcerホットスポット/Bluetooth PAN/同じWi-Fiから選択待ち。[操作手順](OPERATOR_PANEL.md) に選んだ接続のAcer側IPv4を明示する起動方法を記載した。
- 実映像の5点座標一致、子供・複数人、30分無中断、異種シーン間の切替、USB復帰、Xiaomi、12時間はHuman Check Required。同一dotsの実20回交代は継続検証記録を参照。
...
- 09:30の試験後は別作業のTouchDesignerが起動していたため待機した。その後ユーザーの「td終了した」を受け、OS上のTD/StreamDiffusion不在も確認して11:03にC922n試験を開始した。今回のタスクから別作業を停止していない。
