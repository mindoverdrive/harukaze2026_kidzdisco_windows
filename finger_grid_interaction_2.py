...
def get_hands_data(frame):
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            x, y = display_utils.normalized_to_stage(index_tip.x, index_tip.y, camera_layout)
            
            # Check pinch (distance between thumb and index)
            tx, ty = display_utils.normalized_to_stage(thumb_tip.x, thumb_tip.y, camera_layout)
            dist_sq = (x - tx)**2 + (y - ty)**2
            is_pinching = dist_sq < 1600 # 40*40
            
            hands_data.append({'pos': (x, y), 'pinching': is_pinching})
            
    return hands_data, stage_frame
...
while running:
    ...
    # Camera processing
    ret, frame = cap.read()
    hands_list, stage_frame = get_hands_data(frame) if ret else ([], None)
