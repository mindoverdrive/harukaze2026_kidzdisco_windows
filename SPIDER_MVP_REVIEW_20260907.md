...
- ユーザーが修正後の約1分確認（指を左右・円状に動かし、以前どおり追従、目立つ停止・画面消失・再出現がない）に `ok` と回答。修正後の基本的な見え方を成人確認済みとする。任意の読取失敗瞬間を目視した証拠とはしない。
- 目視trial `kids_trial_20260907_034242_304284400` はこちらのManager終了操作で03:46:54にoperator_quit、exit0、245.016秒、promotion_count1。対象4 PIDと共有メモリ不在を確認。
- 次はParticle Stormの未確認の四隅だけ。Spiderの両手・四隅・退出再入場など既存合格項目は繰り返さない。終了時例外・異種切替・子供/2人・現場条件/12時間は引き続き保留。
...
- 終了後の対象PIDと共有メモリは不在。単一シーン30分無中断の合格とはしない。
- Spiderだけ、一時的なread失敗を60Hz上限で最大1秒再試行する。正常フレームへ復帰すれば継続、継続失敗はcamera_read_failed_timeoutで終了。待機中もイベント処理を続ける。最後の描画を保持するため、その間の映像停止は最大1秒。物理カメラ所有・共通read実装・演出は変更しない。
- 実際のmainループをカメラ・UIの代替入力で実行し、一時復帰、継続失敗の上限、待機中Esc、カメラ/検出器/pygame解放を検証。実機での修正後30分と再発状況は未確認。
