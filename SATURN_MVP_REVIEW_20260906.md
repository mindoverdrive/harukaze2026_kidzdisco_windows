## 2026-09-07 06:06 更新：修正後の目視合格

ユーザーが直前の確認に `ok` と回答。Acer側を操作してもXiaomiにタスクバーが出ず、中央の指に金色カーソルと近くの球の粒子が追従することを今回の目視合格として記録する。対象は既述の055537 trial。既存の両手などの合格とは別に、GLFW配置修正の影響確認を完了した。
次の独立試験のため06:02:58に通常終了。operator_quit / exit0 / trial_elapsed 432.0秒 / promotion1。対象4 PIDの稼働なし、SHM不在を確認。この短い再確認を15分・30分耐久の完走とは記録しない。

# Saturn実機確認

## 2026-09-07 05:57：GLFW配置順修正の影響だけ確認待ち

- `096bace` の共通GLFW配置順修正はSaturnにも適用されるため、Acer側をクリックしてもXiaomiにタスクバーが出ず、中央の指と金色カーソル/近くの球の粒子が追従するかだけ再確認する。結果待ち。両手/pinch/退出再入場など既存合格を一律にやり直さない。
- 先行混在試験の通常終了と対象PID/SHM不在を確認してから、既存 `--audience --scene saturn --duration-minutes 15` で起動。trial `kids_trial_20260907_055537_173816900`、run `saturn_reentry_20260907_055535_820485`、Manager19580/scene23484/wrapper10480/外側19508、SHM `harukaze_cam_19580_e14b21468826`。FIRST_FRAME3.781秒、露出-4/zoom176を一時適用し読戻し一致、保存なし。06:11頃の上限終了。
- 05:56:15のWin32 probeではclient/outerがXiaomi全面(1920,0)〜(3840,1080)に一致し、下端3点ともSaturnをヒット。これは人間の表示合格の代わりではない。36.062秒時点camera failure/reopen0、描画/検出エラーなし、SceneMetrics26.7fps。新規コード変更なし。
