...
def main():
    ...
    while running:
        ...
        if results.multi_hand_landmarks:
            ...
            target1 = np.array(display_utils.normalized_to_stage(h0.x, h0.y, camera_layout), dtype=float)
            
            if h_count > 1:
                h1 = results.multi_hand_landmarks[1].landmark[8]
                target2 = np.array(display_utils.normalized_to_stage(h1.x, h1.y, camera_layout), dtype=float)
            else:
        ...
        if target1 is not None:
            pygame.draw.circle(screen, (255, 255, 255), (int(target1[0]), int(target1[1])), 10, 2)
        if target2 is not None:
